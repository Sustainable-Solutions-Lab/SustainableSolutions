...
| Contact / profile links on the active-member card | Four optional Sheet columns: `email`, `scholar_url`, `orcid` (the iD only — the URL prefix is added at render), and `cv_url` (Google Drive link or hosted PDF). The active-member card on `/people` renders an action row: Profile · CV · Email · Website · Scholar · ORCID — only the ones with values appear. |
