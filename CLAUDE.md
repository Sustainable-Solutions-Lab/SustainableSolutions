...
| Q      | notable_pub_doi   | Text                                           | DOI of the paper to feature as "Notable work". Falls back to most-cited if blank. Also accepts `tool:<slug>` (e.g. `tool:cornerstone`) to surface a Tools-tab entry instead of a paper. |
